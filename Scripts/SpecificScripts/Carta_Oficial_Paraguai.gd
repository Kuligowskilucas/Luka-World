...
onready var dialogue_control = get_node("/root/Main_3/CanvasLayer/DialogueControl")
...
func _on_body_entered(body):
	if body.is_in_group("player"):
		...
		if dialogue_control:
			...
			dialogue_control.start_dialogue(Dialogues.dialogues["Carta_Oficial_Paraguai"])
		else:
			print("Controlador de Diálogo não encontrado!")
...
func _on_dialogue_finished():
	queue_free()
	get_node("/root/Main_3").on_collectable_collected_3()
